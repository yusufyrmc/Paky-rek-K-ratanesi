...
async function runSupabase(sql, params = []) {
  ...
  if (/^INSERT INTO payments \(table_id, table_name, amount, payment_type, waiter_name, created_at\) VALUES \(\?, \?, \?, \?, \?, datetime\('now', 'localtime'\)\)$/i.test(s)) {
    return sbInsert('payments', {
      table_id: params[0],
      table_name: params[1],
      amount: params[2],
      payment_type: params[3],
  ...
  if (/^DELETE FROM payments WHERE id = \? AND payment_type = 'veresiye'$/i.test(s)) {
    return sbDelete('payments', (q) => q.eq('id', params[0]).eq('payment_type', 'veresiye'));
  }
  ...
  if (/^UPDATE orders SET total_amount = \? WHERE id = \?$/i.test(s)) {
    return sbUpdate('orders', { total_amount: params[0] }, (q) => q.eq('id', params[1]));
  }
...
async function allSupabase(sql, params = []) {
  ...
  if (/^SELECT id, section FROM tables ORDER BY section, id$/i.test(s)) {
    const { data, error } = await supabase
      .from('tables')
      .select('id, section')
      .order('section', { ascending: true })
      .order('id', { ascending: true });
    throwIf(error, 'masa sıralaması');
  ...
  if (s.includes('FROM payments') && s.includes('created_at >= ?') && s.includes('created_at < ?')) {
    const selectedColumns = s.match(/^SELECT (.+?) FROM payments/i)?.[1] || '*';
    let query = supabase
      .from('payments')
      .select(selectedColumns)
      .gte('created_at', params[0])
      .lt('created_at', params[1]);
    if (s.includes('ORDER BY created_at DESC')) query = query.order('created_at', { ascending: false });
    const limit = s.match(/LIMIT (\d+)/i);
    if (limit) query = query.limit(Number(limit[1]));
    const { data, error } = await query;
    throwIf(error, 'tarih aralığı ödemeleri');
    return coerceRows(data);
  }
  if (s.includes('FROM payments') && s.includes("payment_type = 'veresiye'") && s.includes('LIMIT 200')) {
    const { data, error } = await supabase
      .from('payments')
      .select('id, table_id, table_name, amount, waiter_name, created_at')
      .eq('payment_type', 'veresiye')
      .order('created_at', { ascending: false })
      .limit(200);
    throwIf(error, 'masa veresiye kayıtları');
    return coerceRows(data);
  }
...
async function getSupabase(sql, params = []) {
  ...
  if (s.includes('FROM merchant_transactions') && s.includes('WHERE id = ? AND merchant_id = ?') && s.includes("type = 'order'")) {
    const { data, error } = await supabase
      .from('merchant_transactions')
      .select('*')
      .eq('id', params[0])
  ...
  if (s.includes('FROM payments') && s.includes('created_at >= ?') && s.includes('created_at < ?') && s.includes('SUM(amount)')) {
    const { data, error } = await supabase
      .from('payments')
      .select('amount, payment_type')
      .gte('created_at', params[0])
      .lt('created_at', params[1]);
    throwIf(error, 'iş günü cirosu');
    const rows = data || [];
  ...
  if (s.includes('FROM payments') && s.includes("payment_type = 'veresiye'") && s.includes("date(created_at) = date('now'")) {
    ...
    const { data, error } = await supabase
      ...
      .lte('created_at', end);
    throwIf(error, 'bugünkü masa veresiyesi');
    return { amount: (data || []).reduce((sum, row) => sum + Number(row.amount || 0), 0) };
  }
