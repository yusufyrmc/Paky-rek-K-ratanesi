    ...
    const tables = await all(`
      SELECT t.*, 
        COALESCE((
          ...
          FROM orders o 
          WHERE o.table_id = t.id AND o.status IN ('approved', 'preparing', 'ready')
        ), 0) as current_total,
        COALESCE((
          SELECT COUNT(*) 
          FROM orders o 
...
app.post('/api/tables', async (req, res) => {
  try {
    ...
    const result = await run('INSERT INTO tables (name, default_name, section, custom_tea_price, is_special) VALUES (?, ?, ?, ?, ?)', [name, name, section || 'Salon', teaPrice, specialVal]);
...
async function resetTableToDefaultName(tableId) {
  const tables = await all('SELECT id, section FROM tables ORDER BY section, id');
  const targetTable = tables.find(table => Number(table.id) === Number(tableId));
  if (!targetTable) return;

  // Unique masa adları nedeniyle önce geçici isimlerle çakışmaları kaldır.
  const stamp = Date.now();
  for (const table of tables) {
    await run('UPDATE tables SET name = ? WHERE id = ?', [`__masa_reset_${stamp}_${table.id}`, table.id]);
  }
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    // Garson siparişi doğrudan masanın açık adisyonuna kaydet
    ...
    // Sipariş nesnesini tam olarak çekip kasa ve garson ekranlarına ilet
    ...
    // CANLI YAYIN: Kasa ve garson ekranlarına anında haber ver!
...
app.post('/api/tables/:id/pay', async (req, res) => {
  try {
    ...
    const totalAmount = openOrders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0);
    const requestedAmount = req.body.amount === undefined || req.body.amount === null || req.body.amount === ''
      ? totalAmount
      : Number(req.body.amount);

    if (!Number.isFinite(requestedAmount) || requestedAmount <= 0) {
      return res.status(400).json({ success: false, error: 'Geçerli bir ödeme tutarı girilmelidir.' });
    }
    if (!['nakit', 'kart', 'veresiye'].includes(payment_type || 'nakit')) {
      return res.status(400).json({ success: false, error: 'Geçersiz ödeme türü.' });
    }
    if (requestedAmount > totalAmount + 0.005) {
      return res.status(400).json({ success: false, error: `Ödeme tutarı hesap toplamını aşamaz (${totalAmount.toFixed(2)} ₺).` });
    }
    ...
    `, [table.id, table.name, requestedAmount, payment_type || 'nakit', waiter_name || 'Kasa']);

    if (requestedAmount >= totalAmount - 0.005) {
      // Hesabın tamamı ödendiyse masadaki açık siparişleri kapat.
      await run(`
        UPDATE orders SET status = 'completed', updated_at = datetime('now', 'localtime')
        WHERE table_id = ? AND status IN ('approved', 'preparing', 'ready')
      `, [id]);
    } else {
      // Kısmi ödemeyi siparişlere sırayla uygula; kalan tutar masada kalır.
      let remainingPayment = requestedAmount;
      for (const order of openOrders) {
        if (remainingPayment <= 0.005) break;
        const orderTotal = Number(order.total_amount || 0);
        if (remainingPayment >= orderTotal - 0.005) {
          await run("UPDATE orders SET status = ?, updated_at = datetime('now', 'localtime') WHERE id = ?", ['completed', order.id]);
          remainingPayment -= orderTotal;
        } else {
          await run('UPDATE orders SET total_amount = ? WHERE id = ?', [Math.max(0, orderTotal - remainingPayment), order.id]);
          remainingPayment = 0;
        }
      }
    }

    const remainingOrders = await all(`
      SELECT * FROM orders
      WHERE table_id = ? AND status IN ('approved', 'preparing', 'ready')
    `, [id]);
    const remainingAmount = remainingOrders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0);
    ...
    res.json({
      ...
      remainingAmount,
      message: remainingAmount > 0.005 ? 'Kısmi ödeme alındı.' : 'Hesap başarıyla kapatıldı.'
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
function formatBusinessDayKey(date) {
...
function shiftBusinessDayKey(dateKey, offset) {
  const [year, month, day] = dateKey.split('-').map(Number);
  const shifted = new Date(Date.UTC(year, month - 1, day + offset));
  return `${shifted.getUTCFullYear()}-${String(shifted.getUTCMonth() + 1).padStart(2, '0')}-${String(shifted.getUTCDate()).padStart(2, '0')}`;
}

function getBusinessDayWindow(reference = new Date()) {
  ...
  const currentDateKey = formatBusinessDayKey(reference);
  const businessDateKey = hour < 1 || (hour === 1 && minute < 40)
    ? shiftBusinessDayKey(currentDateKey, -1)
    : currentDateKey;
  const nextDateKey = shiftBusinessDayKey(businessDateKey, 1);

  return {
    businessDate: new Date(`${businessDateKey}T00:00:00+03:00`),
    start: new Date(`${businessDateKey}T01:40:00+03:00`),
    end: new Date(`${nextDateKey}T01:40:00+03:00`)
  };
}

function formatReportBoundary(date) {
  if (!db) return date.toISOString();
...
function formatBusinessDayLabel(date) {
...
app.get('/api/reports/daily', async (req, res) => {
  try {
    const currentWindow = getBusinessDayWindow();
    const currentStart = formatReportBoundary(currentWindow.start);
    const currentEnd = formatReportBoundary(currentWindow.end);

    // İş günü İstanbul saatiyle 01:40'ta değişir.
    const currentBusinessDateKey = formatBusinessDayKey(currentWindow.businessDate);
    const [businessYear, businessMonth, businessDay] = currentBusinessDateKey.split('-').map(Number);
    const weekday = new Date(Date.UTC(businessYear, businessMonth - 1, businessDay)).getUTCDay();
    const mondayOffset = (weekday + 6) % 7;
    const weekStartKey = shiftBusinessDayKey(currentBusinessDateKey, -mondayOffset);
    const weekEndKey = shiftBusinessDayKey(weekStartKey, 7);
    const weekStart = new Date(`${weekStartKey}T01:40:00+03:00`);
    const weekEnd = new Date(`${weekEndKey}T01:40:00+03:00`);

    const [totalRevenue, topProducts, recentPayments, paymentHistory] = await Promise.all([
      get(`
      SELECT 
      ...
      WHERE created_at >= ? AND created_at < ?
      ORDER BY created_at ASC
      `, [formatReportBoundary(weekStart), formatReportBoundary(weekEnd)])
    ]);

    const groupedByBusinessDay = new Map();
    for (const payment of paymentHistory) {
      const paymentDate = new Date(payment.created_at);
      const paymentWindow = getBusinessDayWindow(paymentDate);
      const key = formatBusinessDayKey(paymentWindow.businessDate);
      const current = groupedByBusinessDay.get(key) || { total: 0, transaction_count: 0 };
      current.total += Number(payment.amount || 0);
      current.transaction_count += 1;
      groupedByBusinessDay.set(key, current);
    }

    const lastSevenBusinessDays = [];
    for (let offset = 0; offset < 7; offset += 1) {
      const key = shiftBusinessDayKey(weekStartKey, offset);
      const summary = groupedByBusinessDay.get(key) || { total: 0, transaction_count: 0 };
      const dayParts = new Intl.DateTimeFormat('tr-TR', {
        day: '2-digit',
        month: '2-digit',
        weekday: 'long',
        timeZone: 'Europe/Istanbul'
      }).formatToParts(new Date(`${key}T12:00:00+03:00`));
      const weekdayLabel = dayParts.find((part) => part.type === 'weekday').value;
      const dayNumber = dayParts.find((part) => part.type === 'day').value;
      const monthNumber = dayParts.find((part) => part.type === 'month').value;
      lastSevenBusinessDays.push({
        label: `${weekdayLabel.charAt(0).toLocaleUpperCase('tr-TR')}${weekdayLabel.slice(1)} ${dayNumber}.${monthNumber}`,
...
// Esnaf ve masalardaki veresiye kayıtlarını birlikte getir
app.get('/api/credit-summary', async (req, res) => {
  try {
    const tableCredits = await all(`
      SELECT id, table_id, table_name, amount, waiter_name, created_at
      FROM payments
      WHERE payment_type = 'veresiye'
      ORDER BY created_at DESC
      LIMIT 200
    `);
    const tableCreditTotal = tableCredits.reduce((sum, item) => sum + Number(item.amount || 0), 0);
    const todayTableCredits = await get(`
      SELECT COALESCE(SUM(amount), 0) AS amount
      FROM payments
      WHERE payment_type = 'veresiye' AND date(created_at) = date('now', 'localtime')
    `);
    ...
    res.json({
      ...
      tableCredits,
      tableCreditTotal,
      todayTableCredits: Number(todayTableCredits.amount || 0)
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
// Masa veresiye kaydını sil
app.delete('/api/credit-summary/table/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await run("DELETE FROM payments WHERE id = ? AND payment_type = 'veresiye'", [id]);
    if (!result.changes) {
      return res.status(404).json({ success: false, error: 'Masa veresiye kaydı bulunamadı.' });
    }

    io.emit('credit_changed');
    res.json({ success: true, message: 'Masa veresiye kaydı silindi.' });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});
...
// Esnaftan tahsilat alma
app.post('/api/merchants/:id/pay', async (req, res) => {
  try {
    const { id } = req.params;
    const amount = Number(req.body.amount);
    const paymentType = req.body.payment_type || 'nakit';
    const note = String(req.body.note || '').trim();
    const waiterName = req.body.waiter_name || 'Esnaf Paneli';

    if (!Number.isFinite(amount) || amount <= 0) {
      return res.status(400).json({ success: false, error: 'Geçerli bir tahsilat tutarı girilmelidir' });
    }
    if (!['nakit', 'kart'].includes(paymentType)) {
      return res.status(400).json({ success: false, error: 'Geçersiz ödeme türü' });
    }
    ...
    `, [id, amount, note || 'Tahsilat', paymentType, waiterName]);
    const balance = await refreshMerchantBalance(id);

    io.emit('merchants_changed');
    res.json({ success: true, balance, message: `${amount.toFixed(2)} ₺ tahsilat kaydedildi` });
  } catch (error) {
...
app.delete('/api/merchants/:merchantId/transactions/:transactionId', async (req, res) => {
  try {
    ...
    const deleted = await run('DELETE FROM merchant_transactions WHERE id = ?', [transactionId]);
    if (!deleted.changes) {
      return res.status(409).json({ success: false, error: 'Sipariş kaydı silinemedi; lütfen tekrar deneyin' });
    }
