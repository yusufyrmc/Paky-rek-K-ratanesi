...
async function initSqliteSchema() {
  ...
  try {
    await runSqlite("ALTER TABLE orders ADD COLUMN charged_at DATETIME DEFAULT NULL");
  } catch (e) {}
  ...
  await runSqlite('CREATE INDEX IF NOT EXISTS idx_payments_created_at ON payments (created_at)');
  await runSqlite('CREATE INDEX IF NOT EXISTS idx_orders_created_at_status ON orders (created_at, status)');
  await runSqlite('CREATE INDEX IF NOT EXISTS idx_order_items_order_product ON order_items (order_id, product_name)');
}
...
async function seedDefaultRows() {
  ...
  if (tableCount.count === 0) {
    ...
    // İçerisi Masaları 1 - 25
    for (let i = 1; i <= 25; i++) {
      await run('INSERT INTO tables (name, default_name, section, status) VALUES (?, ?, ?, ?)', [`Masa ${i}`, `Masa ${i}`, 'İçerisi', 'empty']);
    }
    // Bahçe Masaları 1 - 10
    for (let i = 1; i <= 10; i++) {
      await run('INSERT INTO tables (name, default_name, section, status) VALUES (?, ?, ?, ?)', [`Bahçe ${i}`, `Bahçe ${i}`, 'Bahçe', 'empty']);
    }
    // Dışarısı Masaları 1 - 5
    for (let i = 1; i <= 5; i++) {
      await run('INSERT INTO tables (name, default_name, section, status) VALUES (?, ?, ?, ?)', [`Dışarısı ${i}`, `Dışarısı ${i}`, 'Dışarısı', 'empty']);
    }
  }
