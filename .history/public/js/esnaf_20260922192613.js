...
let customOrderSubmissionInProgress = false;
...
async function submitCustomOrder() {
  if (!selectedMerchantForOrder || customOrderSubmissionInProgress) return;
  ...
  customOrderSubmissionInProgress = true;
  const submitButton = document.querySelector('#customOrderModal button[onclick="submitCustomOrder()"]');
  if (submitButton) {
  ...
  try {
    ...
    customOrderSubmissionInProgress = false;
    if (submitButton) {
...
function openDeleteMerchantModal() {
  const modal = document.getElementById('deleteMerchantModal');
  const list = document.getElementById('deleteMerchantList');
  if (!modal || !list) return;

  if (allMerchants.length === 0) {
    list.innerHTML = '<div style="text-align: center; color: var(--text-secondary); padding: 20px;">Kayıtlı esnaf bulunmuyor.</div>';
  } else {
    list.innerHTML = allMerchants.map(merchant => `
      <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 10px 12px; background: rgba(255,255,255,0.04); border: 1px solid var(--border-color); border-radius: 8px;">
        <div>
          <div style="font-weight: 700; color: #fff;">${escapeHtml(merchant.name)}</div>
          <div style="font-size: 0.8rem; color: var(--text-secondary);">${escapeHtml(merchant.shop_type || 'Esnaf')} · ${Number(merchant.balance || 0).toFixed(2)} ₺</div>
        </div>
        <button class="btn btn-outline" style="padding: 6px 10px; color: #fca5a5; border-color: rgba(239,68,68,0.45);" onclick="deleteMerchant(${merchant.id}, '${escapeHtml(merchant.name)}')">Sil</button>
      </div>
    `).join('');
  }
