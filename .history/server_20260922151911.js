...
app.get('/api/menu', async (req, res) => {
  try {
    ...
    const products = await all(`
      ...
      WHERE is_active = 1
      ORDER BY category_id, sort_order ASC, CASE WHEN LOWER(name) = 'çay' THEN 0 ELSE 1 END, name ASC
    `);
...
app.put('/api/products/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { category_id, name, price, quick_notes, is_active, special_price, sort_order } = req.body;
    const notesStr = quick_notes ? JSON.stringify(quick_notes) : '[]';
    const specPrice = (special_price !== undefined && special_price !== '' && special_price !== null)
      ? parseFloat(special_price)
      : null;
    ...
    await run(
      'UPDATE products SET category_id = ?, name = ?, price = ?, sort_order = ?, quick_notes = ?, is_active = ?, special_price = ? WHERE id = ?',
      [category_id, name, parseFloat(price), Number.isFinite(Number(sort_order)) ? parseInt(sort_order, 10) : 0, notesStr, is_active ?? 1, specPrice, id]
    );
    io.emit('menu_changed');
    res.json({ success: true });
  } catch (error) {
...
app.delete('/api/merchants/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await run('DELETE FROM merchant_product_prices WHERE merchant_id = ?', [id]);
