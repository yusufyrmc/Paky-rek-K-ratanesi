...
app.get('/api/reports/daily', async (req, res) => {
  try {
    ...
    const historyStartDate = new Date(`${shiftBusinessDayKey(formatBusinessDayKey(currentWindow.businessDate), -6)}T01:40:00+03:00`);
    const paymentHistory = await all(`
      SELECT created_at, amount
      FROM payments
      WHERE created_at >= ? AND created_at < ?
      ORDER BY created_at ASC
    `, [formatReportBoundary(historyStartDate), currentEnd]);
