...
app.get('/api/reports/daily', async (req, res) => {
  try {
    ...
    const currentStartIso = currentWindow.start.toISOString();
    const currentEndIso = currentWindow.end.toISOString();

    // Günün iş yeri dönemi: 01:50'de sıfırlanır
    ...
    const historyStartDate = new Date(currentWindow.businessDate);
    historyStartDate.setDate(historyStartDate.getDate() - 6);
    historyStartDate.setHours(1, 50, 0, 0);

    const historyEndDate = new Date(currentWindow.end);
    const paymentHistory = await all(`
      SELECT created_at, amount
      FROM payments
      WHERE created_at >= ? AND created_at < ?
      ORDER BY created_at ASC
    `, [historyStartDate.toISOString(), historyEndDate.toISOString()]);
