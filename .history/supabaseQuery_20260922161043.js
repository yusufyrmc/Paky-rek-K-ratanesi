...
async function runSupabase(sql, params = []) {
  ...
  if (/^INSERT INTO merchant_product_prices \(merchant_id, product_id, custom_price\) VALUES \(\?, \?, \?\) ON CONFLICT\(merchant_id, product_id\) DO UPDATE SET custom_price = excluded\.custom_price, updated_at = CURRENT_TIMESTAMP$/i.test(s)) {
...
async function allSupabase(sql, params = []) {
  ...
  if (/^SELECT \* FROM products WHERE is_active = 1 ORDER BY category_id, (CASE WHEN LOWER\(name\) = 'çay' THEN 0 ELSE 1 END, )?name ASC$/i.test(s)) {
...
async function getSupabase(sql, params = []) {
  ...
  if (/^SELECT custom_price FROM merchant_product_prices WHERE merchant_id = \? AND product_id = \?$/i.test(s)) {
    const { data, error } = await supabase
      .from('merchant_product_prices')
      .select('custom_price')
      .eq('merchant_id', params[0])
