...
CREATE TABLE IF NOT EXISTS payments (
  ...
  waiter_name TEXT,
  business_date DATE DEFAULT (((CURRENT_TIMESTAMP AT TIME ZONE 'Europe/Istanbul') - INTERVAL '1 hour 40 minutes')::date),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Herkes daily_revenues okuyup yazabilir') THEN
...
UPDATE tables SET default_name = name WHERE default_name IS NULL OR default_name = '';
