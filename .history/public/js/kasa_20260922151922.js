      ...
      html += `
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; background: rgba(255,255,255,0.04); border-radius: 6px; margin-bottom: 8px; flex-wrap: wrap; gap: 8px;">
          ...
          <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
            <button class="btn btn-outline" style="padding: 6px 9px;" onclick="moveProduct(${p.id}, ${cat.id}, ${index - 1})" ${index === 0 ? 'disabled' : ''} title="Yukarı taşı">↑</button>
            <button class="btn btn-outline" style="padding: 6px 9px;" onclick="moveProduct(${p.id}, ${cat.id}, ${index + 1})" ${index === cat.products.length - 1 ? 'disabled' : ''} title="Aşağı taşı">↓</button>
...
async function moveProduct(productId, categoryId, targetIndex) {
  const category = rawMenuData.find(cat => cat.id === categoryId);
  if (!category || targetIndex < 0 || targetIndex >= category.products.length) return;

  const currentIndex = category.products.findIndex(product => product.id === productId);
  if (currentIndex === -1 || currentIndex === targetIndex) return;

  const reordered = [...category.products];
  const [movedProduct] = reordered.splice(currentIndex, 1);
  reordered.splice(targetIndex, 0, movedProduct);

  try {
    const responses = await Promise.all(reordered.map((product, index) => fetch(`/api/products/${product.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...
        price: product.price,
        sort_order: index,
        quick_notes: product.quick_notes || [],
    ...
    if (responses.every(response => response.ok)) {
      ...
      showToast('Ürün sırası kaydedilemedi', 'danger');
    }
  } catch (err) {
  ...
  try {
    const res = await fetch(`/api/products/${activeProductForNotes.id}`, {
      ...
      body: JSON.stringify({
        ...
        sort_order: activeProductForNotes.sort_order,
  ...
  try {
    const res = await fetch(`/api/products/${prodId}`, {
      ...
      body: JSON.stringify({
        ...
        sort_order: found ? found.sort_order : 0,
