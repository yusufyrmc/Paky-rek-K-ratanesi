...
app.get('/api/reports/daily', async (req, res) => {
  try {
    ...
    const [totalRevenue, topProducts, recentPayments, paymentHistory] = await Promise.all([
      ...
      WHERE created_at >= ? AND created_at < ?
      ORDER BY created_at ASC
      `, [formatReportBoundary(new Date(`${shiftBusinessDayKey(formatBusinessDayKey(currentWindow.businessDate), -6)}T01:40:00+03:00`)), currentEnd])
    ]);
    ...
    for (let offset = 6; offset >= 0; offset -= 1) {
      const day = new Date(currentWindow.businessDate);
