...
async function loadVeresiyeEntries() {
  ...
  try {
    const [merchantsRes, summaryRes] = await Promise.all([
      fetch('/api/merchants'),
    ...
    if (!merchantsData.success || !summaryData.success) {
      if (listContainer) {
...
function renderVeresiyeList(items) {
  const listContainer = document.getElementById('veresiyeList');
