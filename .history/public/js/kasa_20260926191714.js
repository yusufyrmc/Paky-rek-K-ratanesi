  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.topProducts.length === 0) {
        topContainer.innerHTML = `<div style="text-align: center; color: var(--text-secondary); padding: 20px;">Bu iş gününde henüz satış kaydı yok.</div>`;
      } else {
      ...
      if (data.recentPayments.length === 0) {
        paymentsContainer.innerHTML = `<div style="text-align: center; color: var(--text-secondary); padding: 20px;">Bu iş gününde henüz ödeme yok.</div>`;
      } else {
...
function scheduleBusinessDayRefresh() {
  ...
  const nextDayOffset = hour > 1 || (hour === 1 && minute >= 40) ? 1 : 0;
  const resetDate = new Date(Date.UTC(year, month - 1, day + nextDayOffset));
  const resetDateKey = `${resetDate.getUTCFullYear()}-${String(resetDate.getUTCMonth() + 1).padStart(2, '0')}-${String(resetDate.getUTCDate()).padStart(2, '0')}`;
  const nextReset = new Date(`${resetDateKey}T01:40:00+03:00`);

  businessDayRefreshTimer = setTimeout(async () => {
    await loadDailyReports();
    scheduleBusinessDayRefresh();
  }, Math.max(1000, nextReset.getTime() - now.getTime() + 1000));
}
...
function openVeresiyeModal() {
  const modal = document.getElementById('veresiyeModal');
  if (!modal) return;
...
async function loadVeresiyeEntries() {
  const listContainer = document.getElementById('veresiyeList');
  if (listContainer) {
    listContainer.innerHTML = '<div style="text-align: center; color: var(--text-secondary); padding: 24px;">Veresiye verileri yükleniyor...</div>';
  }

  try {
    const [merchantsRes, summaryRes, creditRes] = await Promise.all([
      fetch('/api/merchants'),
    ...
    if (!merchantsData.success || !summaryData.success || !creditData.success) {
      if (listContainer) {
        listContainer.innerHTML = '<div style="text-align: center; color: #ef4444; padding: 24px;">Veresiye verileri alınamadı!</div>';
      }
      return;
    }
    ...
    const todayOrdersEl = document.getElementById('veresiyeTodayOrders');
    ...
    if (listContainer) {
      listContainer.innerHTML = '<div style="text-align: center; color: #ef4444; padding: 24px;">Bağlantı hatası oluştu.</div>';
    }
  }
}

function renderVeresiyeList(items, tableCredits = []) {
  const listContainer = document.getElementById('veresiyeList');
  if (!listContainer) return;

  const searchValue = (document.getElementById('veresiyeSearchInput')?.value || '').trim().toLowerCase();
  const filtered = items.filter(item => {
    const text = `${item.name || ''} ${item.shop_type || ''} ${item.phone || ''} ${item.notes || ''}`.toLowerCase();
    return !searchValue || text.includes(searchValue);
  });

  const filteredTableCredits = tableCredits.filter(item => {
    const text = `${item.table_name || ''} ${item.waiter_name || ''}`.toLowerCase();
    return !searchValue || text.includes(searchValue);
  });

  if (!filtered.length && !filteredTableCredits.length) {
    listContainer.innerHTML = `
      <div style="text-align: center; color: var(--text-secondary); padding: 30px; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(251, 191, 36, 0.4);">
  ...
  const merchantHtml = filtered.map(item => {
    const balance = parseFloat(item.balance) || 0;
    const icon = getMerchantTypeIcon(item.shop_type);
    const lastTx = item.last_tx_time ? new Date(item.last_tx_time).toLocaleString('tr-TR', {
      day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit'
    }) : 'Henüz hareket yok';

    return `
      <div style="display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 12px 14px; background: rgba(255,255,255,0.04); border: 1px solid rgba(251,191,36,0.35); border-radius: 10px; flex-wrap: wrap;">
        <div style="display: flex; align-items: center; gap: 12px; min-width: 0; flex: 1;">
          <div style="font-size: 1.6rem; line-height: 1; background: rgba(251,191,36,0.12); padding: 8px; border-radius: 8px;">${icon}</div>
          <div style="min-width: 0;">
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
              <span style="font-weight: 800; color: #fff; font-size: 1rem;">${escapeHtml(item.name)}</span>
              <span style="font-size: 0.72rem; background: rgba(251,191,36,0.15); color: #fbbf24; padding: 2px 6px; border-radius: 4px; border: 1px solid rgba(251,191,36,0.4);">
                ${escapeHtml(item.shop_type || 'Esnaf')}
              </span>
            </div>
            <div style="font-size: 0.78rem; color: var(--text-secondary); margin-top: 3px; display: flex; flex-wrap: wrap; gap: 10px;">
              ${item.phone ? `<span>📞 ${escapeHtml(item.phone)}</span>` : ''}
              ${item.notes ? `<span>📝 ${escapeHtml(item.notes)}</span>` : ''}
            </div>
            <div style="font-size: 0.72rem; color: #cbd5e1; margin-top: 2px;">Son hareket: ${escapeHtml(lastTx)}</div>
          </div>
        </div>

        <div style="display: flex; align-items: center; gap: 14px;">
          <div style="text-align: right;">
            <div style="font-size: 0.7rem; color: var(--text-secondary); text-transform: uppercase;">Açık Borç</div>
            <div style="font-size: 1.3rem; font-weight: 800; color: #f87171;">${balance.toFixed(2)} ₺</div>
          </div>
          <button class="btn btn-outline" style="padding: 6px 10px; color: #fbbf24; border-color: rgba(251,191,36,0.5); font-size: 0.8rem;" onclick="window.location.href='esnaf.html'" title="Esnaf çetelesini aç">
            📋 Çetele
          </button>
        </div>
      </div>
  ...
  const tableHtml = filteredTableCredits.length ? `
    <div style="font-weight: 700; color: #fff; font-size: 0.95rem; margin: 8px 0 2px;">🪑 Masa Veresiyeleri</div>
    ${filteredTableCredits.map(item => `
      <div style="display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 12px 14px; background: rgba(255,255,255,0.04); border: 1px solid rgba(251,191,36,0.35); border-radius: 10px; flex-wrap: wrap;">
        <div>
  ...
  listContainer.innerHTML = `${merchantHtml}${tableHtml}`;
}

async function deleteTableCredit(paymentId) {
  if (!confirm('Bu masa veresiye kaydı silinsin mi?')) return;

  try {
    const res = await fetch(`/api/credit-summary/table/${paymentId}`, { method: 'DELETE' });
    const data = await res.json();
    if (!data.success) {
      showToast(data.error || 'Veresiye kaydı silinemedi.', 'danger');
      return;
    }

    showToast('Masa veresiye kaydı silindi.', 'success');
    await loadVeresiyeEntries();
  } catch (err) {
    console.error('Masa veresiye silme hatası:', err);
    showToast('Veresiye kaydı silinirken bağlantı hatası oluştu.', 'danger');
  }
}

function filterVeresiyeList() {
  const listContainer = document.getElementById('veresiyeList');
  if (!listContainer) return;

  const searchInput = document.getElementById('veresiyeSearchInput');
  ...
  const items = (kasaMerchantsData || []).filter(item => parseFloat(item.balance) > 0);
  const filtered = items.filter(item => {
    const text = `${item.name || ''} ${item.shop_type || ''} ${item.phone || ''} ${item.notes || ''}`.toLowerCase();
  ...
  fetch('/api/credit-summary')
    .then(res => res.json())
    .then(data => renderVeresiyeList(filtered, data.success ? (data.tableCredits || []) : []))
    .catch(() => renderVeresiyeList(filtered));
}
